...
__author__ = "Julian Löffler"
__license__ = "MIT"

import logging
from logger.logutils.logutils.colorize import ColorizingStreamHandler
...
class RainbowLoggingHandler(ColorizingStreamHandler):
    ...
    def colorize(self, record):
        ...
        who = [self.get_color("green"),
               getattr(record, "module", "") + ':' + getattr(record, "funcName", ""),
               "()",
               self.get_color("black", None, True),
               ":",
               self.get_color("cyan"),
               'L' + str(getattr(record, "lineno", 0))]
